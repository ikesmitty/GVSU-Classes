...
class udpserver{
    ...
    public static void main(String args[]) throws Exception{
		...
		while(true){
			...
			ByteBuffer receiveDataByteBuff = ByteBuffer.wrap(receiveData).order(ByteOrder.BIG_ENDIAN);
		...
		//	printDNSPacket(receiveData, receiveDataByteBuff, receiveData, receiveDataByteBuff);
			...
			String query = getFirstDNSQueryString(receiveData, receiveDataByteBuff);
			byte[] recvData = new byte[1024];
			ByteBuffer bb = ByteBuffer.wrap(recvData).order(BytdeOrder.BIG_ENDIAN):
			boolean mustFindValidAnswer = true;
			while(!cacheHasAnswer && mustFindValidAnswer && savedQueries.size() > 0){
				//Get next IP address for valid answer
				String nextIP = savedQueries.remove(savedQueries.size() - 1);
	...
	private static String getCompressedDNSResponseString(ByteBuffer byteBuf, byte[] data, int index){
		...
		while (numChars > 0){
			...
			if (numChars >= 192){
				...
				offset = (int)byteBuff.getShort(index - 1) & 0x3FFF;
